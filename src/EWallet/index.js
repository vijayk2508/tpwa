    ...
    <div className={styles.container}>
      <div className={styles.wrapper}>
        ...
        <div className={styles.card}>
          <div className={styles.walletheader}>My eWallet</div>
          <div className={styles.dollar}>{balance.toFixed(2)}</div>

          <div className={styles.message}>{msg && <Message msg={msg} />}</div>
        </div>
        ...
        <div className={styles.action}>
          ...
          <button onClick={showModal1} className={styles.button}>
            Top Up
          </button>
        </div>
        ...
        <div className={styles.action}>
          <button
            onClick={() => toggleList("trans")}
            className={`${styles.button} ${
              list === "trans" ? styles.green : ""
            }`}
          >
            Transaction
          </button>
          <button
            onClick={() => toggleList("topup")}
            className={`${styles.button} ${
              list === "topup" ? styles.green : ""
            }`}
          >
            Top Up
          </button>
          ...
          <ul>
            {list === "trans"
              ? transhistory.reverse().map((item, i) => (
                  <li key={i}>
                    {item.datetime} - bought {item.product} @ ${item.price}
                  </li>
                ))
              : topuphistory.reverse().map((item, i) => (
                  <li key={i}>
                    {item.datetime} - top up ${item.amount}
                  </li>
                ))}
          </ul>
        </div>
      ...
      <div className={`${styles.modal} ${modal1 ? styles.show : ""}`}>
        <div className={styles.modalheader}>
          ...
          <div className={styles.modaltitle}>Top Up</div>
          <div></div>
        </div>
        ...
        <div className={styles.modalinput}>
          <input
            ...
            type="number"
            placeholder="Top Amount"
            value={topup}
            onChange={(e) => setTopup(e.target.value)}
          />
          <button className={styles.button} onClick={() => topupAmount(topup)}>
            Confirm
          </button>
        </div>
      </div>
      ...
      <div className={`${styles.modal} ${modal2 ? styles.show : ""}`}>
        <div className={styles.modalheader}>
          ...
          <div className={styles.modaltitle}>Scan &amp; Pay</div>
          <div></div>
        </div>
